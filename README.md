...
```bash
# 生成3张测试地图
python -c "from map.map_generator import MapGenerator; MapGenerator().generate_multiple(3, 'assets/maps')"

# 生成5张地图
python -c "from map.map_generator import MapGenerator; MapGenerator().generate_multiple(5, 'assets/maps')"

# 指定种子生成固定地图（可复现）
python -c "from map.map_generator import MapGenerator; MapGenerator(seed=42).generate(1, 'assets/maps/custom_level')"
```

### 地图格式

地图文件位于 `assets/maps/*.map`，使用字符矩阵表示：
...
- 游戏启动时从 `assets/maps/` 目录加载所有 `.map` 文件
- 通关后自动随机切换到下一张未使用的地图
- 3张地图全部通关后，显示"恭喜通关!"界面
- 按 `R` 键可重新开始游戏（重置地图池）
